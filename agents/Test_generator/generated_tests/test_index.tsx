...
jest.mock('react-native-maps', () => ({
  __esModule: true,
  default: (props: any) => <mock-mapview {...props} />,
  Polyline: (props: any) => <mock-polyline {...props} />,
  Marker: (props: any) => <mock-marker {...props} />,
}));

jest.mock('@react-native-picker/picker', () => ({
  Picker: (props: any) => <mock-picker {...props} />,
}));
...
// Mock csvData
const mockCsvData = `route_id,timestamp,latitude,longitude,speed_kmph,acceleration_mps2,gyroscope_yaw,gyroscope_roll
1,2024-01-01T00:00:00Z,43.7315,-79.7624,50,0,0,0
1,2024-01-01T00:00:10Z,43.7316,-79.7623,60,1,10,5
1,2024-01-01T00:00:20Z,43.7317,-79.7622,70,-1,-10,-5`;

jest.mock('../../synthetic_telematics_data', () => ({
  csvData: mockCsvData,
}));
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {markerPosition && (
          <mock-marker coordinate={markerPosition} title="Simulated Position" />
        )}
      </mock-mapview>
      <View style={styles.rowContainer}>
        <View style={styles.pickerContainer}>
          ...
          >
            <mock-picker.Item label="Select a route" value={null} />
            {routeData.map((route) => (
              <mock-picker.Item
                key={route.routeId}
...
describe('App Component', {
  ...
  it('renders the map and initial UI elements', () => {
    render(<App />);
    expect(screen.getByTestId('mock-mapview')).toBeDefined();
  ...
  it('fetches route data on mount and updates state', async () => {
    ...
    const mockRouteData = [
      { routeId: '123', coordinates: [{ latitude: 1, longitude: 2 }] },
      { routeId: '456', coordinates: [{ latitude: 3, longitude: 4 }] },
    ];
    jest.spyOn(global, 'fetch').mockImplementation(
      () =>
        Promise.resolve({
          json: () =>
            Promise.resolve({
              features: mockRouteData.map((route) => ({
                properties: { route_id: route.routeId },
                geometry: {
                  type: 'LineString',
    ...
    await waitFor(() => {
      expect(screen.getByText('Route 123')).toBeDefined(); // Check if route data is displayed.
      expect(screen.getByText('Route 456')).toBeDefined();
    });

    expect(fetch).toHaveBeenCalledTimes(1);
  });

  it('handles route selection correctly', async () => {
    const mockRouteData = [
        { routeId: '123', coordinates: [{ latitude: 1, longitude: 2 }] },
        { routeId: '456', coordinates: [{ latitude: 3, longitude: 4 }] },
      ];
    ...
    render(<App />);
    await waitFor(() => {
        expect(screen.getByText('Route 123')).toBeDefined();
        expect(screen.getByText('Route 456')).toBeDefined();
    });

    fireEvent(screen.getByTestId('mock-picker'), 'onValueChange', '123'); // Select a route.  This is NOT how to do this.
    // Check if the selected route is correctly updated in state
    await waitFor(() => {
        // Can't check state directly, but check UI updates.  Not ideal.
      });
  });

  it('starts and stops the simulation correctly', async () => {
      const mockRouteData = [
        { routeId: '1', coordinates: [{ latitude: 1, longitude: 2 }, {latitude: 1.1, longitude: 2.1}] },
      ];
      ...
      fireEvent(screen.getByTestId('mock-picker'), 'onValueChange', '1');

    // Start simulation
    act(() => {
      fireEvent.press(screen.getByText('Start Simulation'));
    });

    // Check if marker is rendered (this is a proxy for the simulation running).
    await waitFor(() => {
        expect(screen.getByTestId('mock-marker')).toBeDefined();
    });
  });

  it('handles errors during route data fetching', async () => {
    jest.spyOn(global, 'fetch').mockImplementation(() =>
    ...
    await waitFor(() => {
      expect(Alert.alert).toHaveBeenCalledWith(
        'Error',
        'Failed to fetch route data. Please try again later.',
      );
    });
  });

  it('correctly calculates route distances', () => {
    const coordinates = [{ latitude: 0, longitude: 0 }, { latitude: 1, longitude: 1 }];
    const component = render(<App />);
    const instance = component.getByTestId('mock-mapview');
    // Access the calculateRouteDistances function
    const calculateRouteDistances = instance.calculateRouteDistances;
    if (!calculateRouteDistances) {
        throw new Error("calculateRouteDistances not found.  Check mock setup.");
    }

    const result = calculateRouteDistances(coordinates);
    expect(result.distances.length).toBe(1);
    expect(result.totalDistance).toBe(100); // as mocked by haversine
  });

  it('correctly gets position along the route', () => {
      const coordinates = [{ latitude: 0, longitude: 0 }, { latitude: 1, longitude: 1 }];
      const distances = [100];
      ...
      const getPositionAlongRoute = instance.getPositionAlongRoute;
      if (!getPositionAlongRoute) {
        throw new Error("getPositionAlongRoute not found.  Check mock setup.");
      }
  ...
  it('handles simulation errors (no route selected)', async () => {
    render(<App />);
    act(() => {
      fireEvent.press(screen.getByText('Start Simulation'));
    });
  ...
  it('handles simulation errors (no simulation data)', async () => {
    // Mock fetch to return some test data
    const mockRouteData = [
        { routeId: '123', coordinates: [{ latitude: 1, longitude: 2 }] },
      ];
      ...
      jest.mock('../../synthetic_telematics_data', () => ({
        csvData: "", // empty to simulate no data
      }));
      render(<App />);
      await waitFor(() => {
          expect(screen.getByText('Route 123')).toBeDefined();
      });
      fireEvent(screen.getByTestId('mock-picker'), 'onValueChange', '123');
      act(() => {
        fireEvent.press(screen.getByText('Start Simulation'));
      });
      await waitFor(() => {
  ...
  it('calls console.log for harsh acceleration', async () => {
    ...
    const mockRouteData = [
      ...
      const mockCsvDataWithHarshAcceleration = `route_id,timestamp,latitude,longitude,speed_kmph,acceleration_mps2,gyroscope_yaw,gyroscope_roll
      1,2024-01-01T00:00:00Z,43.7315,-79.7624,50,1.3,0,0
      1,2024-01-01T00:00:10Z,43.7316,-79.7623,60,1.3,10,5`;
    jest.mock('../../synthetic_telematics_data', () => ({
      csvData: mockCsvDataWithHarshAcceleration,
    }));
    ...
    await waitFor(() => {
      expect(console.log).toHaveBeenCalledWith(
        'Harsh Acceleration detected at timestamp:',
